"""Capture real LSP wire-protocol traffic from fortls.

Spawns fortls, sends LSP messages (initialize, didOpen with a .f90 file),
captures stdout including diagnostics.

Usage: uv run python tests/fixtures/capture_fortls_traffic.py
"""
...
SAMPLE_F90 = os.path.join(WORKSPACE_DIR, "src", "main.f90")
OUTPUT_FILE = os.path.join(FIXTURES_DIR, "fortls-session.bin")
CAPTURE_LOG = os.path.join(FIXTURES_DIR, "capture-fortls-log.txt")
...
def main():
    ...
    print("Spawning fortls...", file=sys.stderr)
    ...
    print(f"\nfortls exit code: {proc.returncode}", file=sys.stderr)
    ...
    print(f"Saved capture to: {OUTPUT_FILE}", file=sys.stderr)

    with open(CAPTURE_LOG, "w") as log:
        log.write("=== CAPTURED LSP MESSAGES (fortls) ===\n\n")
        pos = 0
        msg_num = 0
        while pos < len(stdout_data):
            ...
            try:
                ...
                if parsed.get("method") == "textDocument/publishDiagnostics":
                    diags = parsed.get("params", {}).get("diagnostics", [])
